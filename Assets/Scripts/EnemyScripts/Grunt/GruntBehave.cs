...
public class GruntBehave : MonoBehaviour
{
    [Header("References")]
    ...
    [Header("Animation")]
    public Animator eggplantAnimator;

    [Header("Audio")]
    public AudioSource audioSource;
    public AudioClip attack1, attack2;
    public AudioClip chatter1, chatter2;
    ...
    void Start()
    {
        ...
        eggplantAnimator = GetComponent<Animator>();
    }
    ...
    public void CheckPlayer(float detectDistance)
    {
        ...
        {
            eggplantAnimator.SetBool("canPrepare", false);
            canPrepare = false;
        }
    }
    ...
    public void MovementBehave()
    {
        if (!isGrabbed) 
        {
            ...
            if (canAttack && !eggplantAnimator.GetBool("canAttack") == true)
            {
                AttackPlayer();
            ...
            {
                eggplantAnimator.SetBool("canAttack", false);
            }
        }
    }
    public void RoamAround()
    {
        ...
        if (runTime >= execTime)
        {
            eggplantAnimator.SetBool("isChasing", false);
            GoToRandomPoint(navAgent, randomPoint, roamRadius);
            runTime = 0f;
            eggplantAnimator.SetBool("isRoaming", true);
        }
        else
        {
            eggplantAnimator.SetBool("isRoaming", false);
        }
    }
    public void ChasePlayer()
    {
        eggplantAnimator.SetBool("isChasing", true);
        runTime = 0f;
    ...
    public void PrepareAttack()
    {
        ...
        if (prepTime > 0)
        {
            eggplantAnimator.SetBool("canPrepare", true);
    ...
    {
        eggplantAnimator.SetBool("canAttack", true);
    ...
    {
        if (!hasFallen && isGrabbed && !canStand)
        {
            rb.isKinematic = false;
            navAgent.enabled = false;
        }
        else if (hasFallen && isGrabbed && canStand)
        {
            if (!canAttack)
            {
                rb.isKinematic = true;
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.gameObject.layer == 6 && isGrabbed && !canStand)
        {
            hasFallen = true;
            if (hasFallen)
            {
